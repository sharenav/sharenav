...
import net.sharenav.osmToShareNav.area.Triangle;
import net.sharenav.osmToShareNav.model.Bounds;
import net.sharenav.osmToShareNav.model.ConditionTuple;
...
import net.sharenav.osmToShareNav.model.Path;
import net.sharenav.osmToShareNav.model.RouteNode;
import net.sharenav.osmToShareNav.model.Sequence;
...
public class CreateShareNavData implements FilenameFilter {
...
//    				System.out.println("Rename " + file.getName() + " to " + fileRenamed.getName());
	 ...
	 * - For file names only preceded by a single "/", Osm2ShareNav will always assume 
	...
	private String copyMediaToMid(String mediaPath, String destDir, String additionalSrcPath) {
		...
		if (iPos == -1) {
			...
			// check if file exists in current directory of Osm2ShareNav / the style file
			if (! (new File(realMediaPath).exists())) {
				// check if file exists in current directory of Osm2ShareNav / the style file + "/png" or "/sound"
...
//					System.out.println("realMediaPath not found: " + realMediaPath);
					// if not check if we can use the version included in Osm2ShareNav.jar
					if (CreateShareNavData.class.getResource("/media/"  + additionalSrcPath + "/" + mediaPath) == null) {
						...
						/**
						 * Copy the file from Osm2ShareNav.jar to the destination directory
						 */
						try {
							BufferedInputStream bis = new BufferedInputStream(
								CreateShareNavData.class.getResourceAsStream("/media/" + 
										additionalSrcPath + "/"  + mediaPath)
							);
