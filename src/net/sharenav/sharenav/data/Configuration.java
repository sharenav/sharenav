...
import net.sharenav.util.BufferedInputStream;
//#endif
import net.sharenav.util.BufferedReader;
...
/**
 * This class holds all the configurable data (i.e. the settings) that ShareNav has.
 */
public class Configuration {
	...
	/** bit 108: indicate whether sharenav is running (for android which can kill the process and restart it)*/
	...
	/** bit 143: indicate whether sharenav is connected to gps (for android which can kill the process and restart it)*/
	...
	// Gpx Recording modes
	// ShareNav determines adaptive if a trackpoint is written
	...
	public static void read() {
		...
		try {
			...
			if (tms_filecache_path == null) {
				//#if polish.android
				tms_filecache_path = "/sdcard/ShareNav/tiles/";
				//#else
				// tms_filecache_path = "E:\\ShareNav\\tiles\\"; which format?
				tms_filecache_path = "E:/ShareNav/tiles/";
				//#endif
			}

			/* close the record store before accessing it nested for writing
			 * might otherwise cause problems on some devices
			 * see [ sharenav-Bugs-2983148 ] Recordstore error on startup, settings are not persistent 
			 */
...
//			}
    ...
    **/
	...
	// TMS maps use a factor of two between scale levels, ShareNav
	// uses 1.5
...
//#endif
	...
	public static void deserialise(InputStream is) throws IOException {
		...
		// below version 2 older ShareNav.cfg files could not be read
		if (version != VERSION && version < 22) {
			throw new IOException(Locale.get("configuration.ConfigVersionMismatch")/*Version of the stored config does not match with current ShareNav*/);
		}
