...
public class Legend {
	 ...
	 * This constant must be in sync with Osm2ShareNav
	 */
	 ...
	 * MUST be in sync with the value that Osm2ShareNav writes. */
	...
	public static void readLegend() throws IOException {
		...
		//#else
		//if (enableEdits) {
		//	throw new IOException("The Map files are enabled for editing, but editing is not compiled into ShareNav." +
		//			"Please use the correct Osm2ShareNav settings to recreate the map or use a correct ShareNav to enable editing");
		//}
		//#endif
...
//#endif
	...
	public static final byte scaleToTile(int scale) {
		if (scale < tileScaleLevel[3]) { 		// 45000 in ShareNav 0.5.0
			return 3;
		} else if (scale < tileScaleLevel[2]) { // 180000 in ShareNav 0.5.0
			return 2;
		} else if (scale < tileScaleLevel[1]) { // 900000 in ShareNav 0.5.0
			return 1;
		}
		return 0;
	}
	...
	public static String getAppVersion() {
		...
		String aV = ShareNav.getInstance().getAppProperty("MIDlet-Version");
		if (aV != null) {
			return aV;
		} else {
			return "??";
