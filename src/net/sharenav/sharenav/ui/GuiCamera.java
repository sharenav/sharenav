...
/*
 * ShareNav - Copyright (c) 2008 Harald Mueller apm at users dot sourceforge dot net 
 * See Copying
 */
 ...
 * directly from with in ShareNav.
 * 
 * The idea is to directly be able to geotag images from within ShareNav
 * for example when survaying for OSM.
 *
 */
public class GuiCamera extends Canvas implements CommandListener, ItemCommandListener, GuiCameraInterface, SelectionListener, ShareNavDisplayable
//#if polish.android
, Camera.PictureCallback
//#endif
 {

	private final Command CANCEL_CMD = new Command(Locale.get("generic.Cancel")/*Cancel*/, ShareNavMenu.BACK, 5);
	private final Command OK_CMD = new Command(Locale.get("generic.OK")/*Ok*/, ShareNavMenu.OK, 5);
	private final Command OK2_CMD = new Command(Locale.get("generic.OK")/*Ok*/, ShareNavMenu.OK, 5);
	...
	// FIXME Maybe can be simplifies with setting up a simple setup/init ShareNavDiscoverable instead of separate
	...
	public void onPictureTaken(byte[] data, Camera camera) {
		...
		try {
			...
			FileConnection fc = (FileConnection)Connector.open(basedirectory + "ShareNav-" + HelperRoutines.formatInt2(idx) + "-" + HelperRoutines.formatSimpleDateSecondNow() + ".jpg");
			while (fc.exists()) {
				fc = (FileConnection)Connector.open(basedirectory + "ShareNav-" + HelperRoutines.formatInt2(idx) + "-" + HelperRoutines.formatSimpleDateSecondNow() + ".jpg");
				idx++;
			}
			fc.create();
	...
	private void takePicture234() throws SecurityException {
		...
		try {
			...
			try {
				...
				snapshot.setFilePrefix("ShareNav-"
	...
	public void commandAction(Command c, Displayable disp) {
		...
		if (c == STORE_CMD) {
			//#if polish.api.fileConnection
				...
				new FsDiscover(parent == null ? (ShareNavDisplayable) setupParent : (ShareNavDisplayable) this,
		...
		if (c == SETUP_CMD) {
			...
			encodingCG.setSelectedIndex(encodingSel, true);
					     ...
					     */
					     ...
					     ShareNav.getInstance().show(setupDialog);
			//#debug trace
