...
import net.sharenav.sharenav.names.SearchNames;
import net.sharenav.sharenav.ui.GuiPoiTypeSelectMenu.PoiTypeSelectMenuItem;
//#if polish.api.bigsearch
//#if polish.api.osm-editing
import net.sharenav.sharenav.ui.GuiOsmPoiDisplay;
import net.sharenav.sharenav.ui.GuiOsmWayDisplay;
//#endif
//#endif
...
public class GuiSearch extends Canvas implements CommandListener,
		      ShareNavDisplayable, InputListener, KeySelectMenuReducedListener, CancelMonitorInterface, SaveButtonListener  {
	...
	public GuiSearch(Trace parent, int action) throws Exception {
		...
		needsPainting = false;
		try {
			ShareNav.getTimer().schedule(timerT, 250, 250);
		} catch (Exception e) {
...
//		System.out.println("got Command " + c);
				...
				Thread t = new Thread(new Runnable() {
					public void run() {
						...
						for (int i = 0; i < names.size(); i++) {
							...
							// This change from "[ sharenav-Patches-3002028 ] Improving full text search" reduces the number of failures
							// see also: http://sourceforge.net/projects/sharenav/forums/forum/677687/topic/3708460
		...
		if (c == FULLT_CMD) {
			...
			Display.getDisplay(ShareNav.getInstance()).setCurrentItem(fulltextSearchField);
			//#endif
			ShareNav.getInstance().show(fulltextForm);			
		}
	}
	...
	private void showPoiTypeForm() {
		try{
			...
			ShareNav.getInstance().show(poiSelectForm);
			//#else
	...
	private void editOSM(SearchResult sr) {
		if (Legend.enableEdits) {
			...
			if (sr.type < 0) {
				...
				// relation id & area way id from Osm2ShareNav to ShareNav
...
//#endif
		...
		if (matchMode() && housenumberTimerTask != null) {
			try {
				...
				ShareNav.getTimer().schedule(housenumberTimerTask, 1500);
			} catch (Exception e) {
	...
	public void pointerPressed(int x, int y) {
		...
		if ( (state == STATE_MAIN || state == STATE_FAVORITES)
			...
			try {
				...
				ShareNav.getTimer().schedule(tapAutoReleaseTimerTask, TAP_AUTORELEASE_DELAY);
			} catch (Exception e) {
