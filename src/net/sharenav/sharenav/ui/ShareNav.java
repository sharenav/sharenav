...
import net.sharenav.midlet.gps.importexport.Jsr211ContentHandlerInterface;
//#endif
...
import net.sharenav.midlet.util.SamsLcdLight;
import net.sharenav.midlet.util.SiemGameLight;
//#endif
...
/**
 * Central class of ShareNav which implements the MIDlet interface.
 */
public class ShareNav extends MIDlet implements CommandListener {
	/** Class variable with the Singleton reference. */
	private volatile static ShareNav instance;
...
//#endif
	...
	protected void destroyApp(boolean arg0) throws MIDletStateChangeException {
		...
		//#debug
		System.out.println("destroy ShareNav");
	}

	protected void pauseApp() {
		//#debug
		System.out.println("Pause ShareNav");
...
//#endif
	...
	protected void startApp() throws MIDletStateChangeException {
		//#debug
		System.out.println("Start ShareNav");
		if (!initDone) {
...
//		}
			instance = this;
			System.out.println("Init ShareNav");
			log = new Logger(this);
			...
			try {
				...
				e.printStackTrace();
				errorMsg = Locale.get("sharenav.FailToLoadBasicConf")/*Failed to load basic configuration! Check your map data source: */
					+ e.getMessage();
			}
			if (!Legend.isValid) {
				if (errorMsg == null) {
					errorMsg = Locale.get("sharenav.FailToLoadBasicConf");
				} 
//#if polish.android
				errorMsg += Locale.get("sharenav.ForAndroidInstall")/*  - For Android, you need to manually install the map (e.g. unzip the J2ME map jar bundle with same settings & version as the .apk) on the SD card or use a special bundle script in tools/ for now*/;
//#endif
			}
			...
			try {
				...
				Class handlerClass =
					Class.forName("net.sharenav.sharenav.importexport.Jsr211Impl");
	...
	public void commandAction(Command c, Displayable d) {
		if (c == Alert.DISMISS_COMMAND) {
			//#debug info
			log.info("ShareNav.commandAction: Alert dismissed");
			bAlertOpen = false;
...
//#endif
	...
	// not used as of 2011-01-27; to minimize ShareNav on platforms which support it
...
//		pauseApp();
//		//according to net wisdom this minimizes ShareNav on many platforms where minimizing
//		//is possible
...
//	}
	...
	public void show(Displayable d) {
		...
		//#endif
			try {
				...
				//#debug info
				log.info("ShareNav.show: Going to display " + d.toString());
		...
		//#ifndef polish.android
			...
			//#debug info
			log.info("ShareNav.show Alert open, postponing " + d.toString());
		}
		//#else
		}
		//#endif
	...
	public static ShareNav getInstance() {
		return instance;
	}
				...
				lightTimer = new Thread(new Runnable() {
					...
					public void run() {
						...
						try {
							...
							while (notInterupted) {
								if (trace != null
									...
									if (Configuration
										...
										Display.getDisplay(
											ShareNav.getInstance()).flashBacklight(1000);
										//#if polish.api.nokia-ui
										// Method to keep the backlight on
							...
							logger.error(Locale.get("sharenav.BacklightAPINotSupported")/*Backlight prodding failed, API not supported: */
									+ ncdfe.getMessage());
						}
					} // run()
				} );
	...
	public void showBackLightLevel() {
		if ( Configuration.getCfgBitState(Configuration.CFGBIT_BACKLIGHT_ON) ) {
			if ( Configuration.getCfgBitState(
				...
				trace.alert( Locale.get("sharenav.Backlight")/*Backlight*/,
					(Configuration.getBackLightLevel() == 100 
						? Locale.get("sharenav.BacklightOnWhileGPS") 
						: Locale.get("sharenav.BacklightPercentWhileGPS", level)
					), 1000 );
			} else {
				String level = Integer.toString(Configuration.getBackLightLevel());
				trace.alert( Locale.get("sharenav.Backlight")/*Backlight*/,
					(Configuration.getBackLightLevel() == 100 
						? Locale.get("sharenav.BacklightOn") 
						: Locale.get("sharenav.BacklightPercent",	level)
					), 1000 );
			}
		} else {
			trace.alert(Locale.get("sharenav.Backlight")/*Backlight*/, 
					Locale.get("sharenav.BacklightOff")/*Backlight off*/, 1000);
		}
		stopBackLightTimer();
		startBackLightTimer();
	}
	...
	public boolean needsFreeingMemory() {
		...
		if (totalMem > phoneMaxMemory) {
			...
			if (phoneMaxMemory > Configuration.getPhoneAllTimeMaxMemory() + 100000) {
				if (trace != null) {
					trace.receiveMessage(Locale.get("sharenav.IncMaxMem")/*Inc maxMem: */ + phoneMaxMemory);
				}
			}
			...
			if ((freeMem < 30000)
				...
				switch(i) {
					...
					case 3:
						...
						if (trace != null) {
							trace.receiveMessage(Locale.get("sharenav.FreeingMem")/*Freeing mem*/);
						}
						return true;
				}
				System.gc();
