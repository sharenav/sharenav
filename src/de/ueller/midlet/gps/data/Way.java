...
//			System.out.println("PPM=" + ppm);
	...
	public void draw(PaintContext pc,
			int w,
		...
		for(int i = 0; i < max; i++) {
			if (i<beforeMax){
				roh2=getParLines(xPoints, yPoints, i+1, w2, l3b, l4b,l3e,l4e);
...
//			pc.g.drawRect(s2.x-1, s2.y-1, 3, 3);
	...
	public IntPoint intersectionPoint(IntPoint p1,IntPoint p2,IntPoint p3,IntPoint p4,IntPoint ret){
		int dx1=p2.x-p1.x;
		int dx2=p4.x-p3.x;
		int dx3=p1.x-p3.x;
		int dy1=p2.y-p1.y;
	...
	private boolean getLineLineIntersection(IntPoint p1,IntPoint p2,IntPoint p3,IntPoint p4,IntPoint ret)
	{
		...
		ret.x = (int)((det(det(x1, y1, x2, y2), x1 - x2,
				det(x3, y3, x4, y4), x3 - x4)/
				det(x1 - x2, y1 - y2, x3 - x4, y3 - y4)) +0.5);
		ret.y = (int)((det(det(x1, y1, x2, y2), y1 - y2,
